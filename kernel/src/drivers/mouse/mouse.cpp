...
void ProcessMousePacket(){
    if (!MousePacketReady) return;
        ...
        if (MousePosition.X > renderer->framebuffer->Width-8) MousePosition.X = renderer->framebuffer->Width-8;

        if (MousePosition.Y < 0) MousePosition.Y = 0;
        if (MousePosition.Y > renderer->framebuffer->Height-16) MousePosition.Y = renderer->framebuffer->Height-16;

        renderer->PutChar('a', MousePosition.X, MousePosition.Y);
