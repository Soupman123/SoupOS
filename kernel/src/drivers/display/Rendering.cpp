...
//public
Renderer::Renderer(Framebuffer* framebufferPri, Framebuffer* framebufferSec, PSF1_FONT* psf1_Font){
...
void Renderer::ClearChar(unsigned int color){
    if (textCursorPosition.X == 0){
        textCursorPosition.X = framebuffer->Width;
        textCursorPosition.Y -= 16;
        if (textCursorPosition.Y < 0) textCursorPosition.Y = 0;
    }

    unsigned int xOff = textCursorPosition.X;
    unsigned int yOff = textCursorPosition.Y;
